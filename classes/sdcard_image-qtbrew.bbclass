...
IMAGE_TYPEDEP_qtbrew-sdimg = "cpio.gz"
...
do_image_qtbrew_sdimg[recrdeps] = "do_build"
...
IMAGE_CMD_qtbrew-sdimg () {

    # Align partitions
    SDIMG_SIZE=$(expr ${BOOT_SPACE} + ${BOOT_START})

    echo "Creating filesystem with Boot partition ${BOOT_SPACE} KiB"
    ...
    parted -s ${SDIMG} unit KiB mkpart primary fat32 ${BOOT_START} $(expr ${BOOT_START} \+ ${BOOT_SPACE} \- 1)
    ...
    mcopy -v -i ${WORKDIR}/boot.img -s ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.cpio.gz ::/initrd.cpio.gz || bbfatal "mcopy cannot copy ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.cpio.gz into boot.img"
    if test -n "${DTS}"; then
    ...
    ln -sf ${IMAGE_NAME}.sdimg ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.sdimg
}
